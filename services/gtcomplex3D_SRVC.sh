...
## Superimpose a pair of structures given a GTcomplex transformation matrix
...
-i <id>        Web service job identifier which also corresponds to the
               ...
               \"query\" and a \"hit_record\" (w/o assignments and alignments)
               sections from a GTcomplex .json output file.
...
supcpl="${dirname}/../bin/supcomplex_wrapper.pl"
...
if [[ ! -f "${supcpl}" ]]; then fail+="ERROR: $ID: Program not found: ${supcpl}\n"; fi
...
if [ -z "\${bfail}" ]; then
  cmd="$supcpl --in \"${inpfile}\" --status \"${statusfile}\""
