...
## (C) 2025 Mindaugas Margelevicius, Institute of Biotechnology, Vilnius University
## Submit a gtcomplex job to a workload manager's queue
...
gtcomplexlogfile="${filename}__gtcomplex_out.log"
reslstfile="${filename}__gtcomplex_out.lst"
resultsfile="${filename}__gtcomplex_out.tar"
errorfile="${filename}.err"

sendemail="${dirname}/../bin/sendemail.pl"
gtcomplexpl="${dirname}/../bin/gtcomplex_wrapper.pl"
...
if [[ ! -f "${gtcomplexpl}" ]]; then fail+="ERROR: $ID: Program not found: ${gtcomplexpl}\n"; fi
...
##
$SUBPROG ${SLURM_OPTS} -n ${ncpus} -t 1440 -J gta-ws-gtcs -o "${shoutfile}" \
...
if [[ -n "${ngpunits}" && ${ngpunits} -gt 0 ]]; then
  ...
  ## NOTE: this is not a general solution since jobs may terminate, making it
  ## NOTE: possible for jobs to start simultaneously
  sleep \$((res*6))
fi
...
if [ -z "\${bfail}" ]; then
  cmd="$gtcomplexpl --in \"${inpfile}\" --opt \"${optfile}\" --status \"${statusfile}\""
  cmd+=" --logfile \"${gtcomplexlogfile}\" --reslst \"${reslstfile}\""
