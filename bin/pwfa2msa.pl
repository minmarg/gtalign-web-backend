...
my  $SBJCTs = '';
my  $WIDTH = -1;
my  $MAXNS = -1;##maximum number of sequences to align
my  $ALNBND_at_end = 0;##alignment boundaries at the end of line
my  $TOLXs = 0;
my  $usage = <<EOIN;
...
-a              Alignment boundaries in the form "(ALN:<start>-<end>)"
                are given at the end of the description line.
...
-N <count>      Maximum number of sequences to include in the resulting MSA.
        By default, all subject sequences are included.
...
unless( AlignTopHits( $INPUT, $OUTPUT, $OUTFMT, $QUERY, $EVALUE, \%HSBJCTs, 
            $MAXNS, $ALNBND_at_end, $TOLXs, $WIDTH, \$processed )) {
...
sub AlignTopHits
{
    ...
    return 0 unless ExtractAlignments( $input, $evalue, $rhSBJCTs, $maxnseqs, $query, $alnbnd_at_end, \%hithash, $rec, \%descrip );
...
##        next if /^$/; ## comment to make eof( IN ) to work
            ...
            eof( IN ))
        {
            ...
            undef $e_val;
            undef $qname;
            last if($maxnseqs && 0 < $maxnseqs && $maxnseqs < $rec);
            next;
        }

        if($alnbnd_at_end) {
            if(!$qname && $last =~ /^>(\S+)\s+(.*)\s*\(ALN:\s*(\d+)\-(\d+)\)\s*$/) {
                $$refdesc{$query} = "$1 $2" unless $$refdesc{$query};
                $qname = $1;
                $querystart = $3;
                $queryend = $4;
                $e_val = 0 unless $e_val;
                if($last =~ /[=\s]([\d\.eE\-\+]+|n\/a)\s*\(ALN:\s*\d+\-\d+\)\s*$/) {
                    $e_val = $1;
                ...
                $e_val = 0 unless $e_val;
                if($sbjct =~ /\s*=\s*([\d\.eE\-\+]+|n\/a)$/) {
                    $e_val = $1;
                    $e_val = "1$e_val" if $e_val =~ /^e/i;
                }
                $sbjct =~ s/\s*(?:Expect|E\-value)\s*=\s*(?:[\d\.eE\-\+]+|n\/a)$//i;
            ...
            if(!$qname && $last =~ /^>(\S+)(?:\s+)(?:\()?(?:ALN:\s+)?(\d+)[\-\s](\d+)(?:\))?(.*)$/) {##11-04
            ...
            elsif( $last =~ /^>(\S+)(?:\s+)(?:\()?(?:ALN:\s+)?(\d+)[\-\s](\d+)(?:\))?(.*)$/) {##11-04
                $sbjct = "$1$4";
                $sbjctstart = $2;
                $sbjctend = $3;
                if($last =~ /[=\s]([\d\.eE\-\+]+|n\/a)$/) {
...
sub PrintAlignedHelperPass1
{
    ...
    if( !($format cmp "Stockholm")) {
        ...
        $sbjct =~ s/^(\S+)\s*(.*)$/$1 ($sbjctstart-$sbjctend) $2${expstr}/ if $rec;
        printf( $reffile "## %s\n", $sbjct );
        $$rinfo .= " $1" if $sbjct =~ /^(\S+)/;
    }
}
