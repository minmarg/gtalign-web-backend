...
## Helper script for superimposing 3D structures aligned by GTcomplex
...
my  $SUPERPOSEprog = File::Spec->catfile($FindBin::Bin,"supcomplex1.pl");
...
sub PreprocessInputFile
{
    ...
    while(<F>) {
        ...
        do {$rfnd = $1; next} if(/\s*"tfm_referenced":\s*(\d),?$/);
        do {$rot = $1; $rot =~ s/\s//g; next} if(/\s*"rotation_matrix_rowmajor":\s*\[(.+)\],?$/);
        do {$tran = $1; $tran =~ s/\s//g; next} if(/\s*"translation_vector":\s*\[(.+)\],?$/);
        if(/\s*"reference_description":\s*"(.+)",?$/) {
            $rname = $1;
        ...
        if(eof(F) || /\s*{"hit_record":\s*{/) {
            do {$hf = 1; next} unless $hf;
            $hsn++;
            ...
            unless($qname) {
                ...
                my $dmod = ($rfnd? $rmod: $qmod);
                my $dinput = ($rfnd? $rname: $qname);
                my $ext = (($dinput =~ /$rexcif/)? $CIFEXT: $PDBEXT);
                $outfile = File::Spec->catfile($resdirname,$resbasename) . "__${hsn}.$ext";

                $$rinputs{"${hsn}_cmdline"} = "$SUPERPOSEprog -i \"$dinput\"";
                $$rinputs{"${hsn}_cmdline"} .= " -o \"$outfile\"";
                $$rinputs{"${hsn}_cmdline"} .= " -m $dmod" if($dmod);
                $$rinputs{"${hsn}_cmdline"} .= " -r $rot -t $tran";
